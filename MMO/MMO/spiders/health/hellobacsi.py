...
class Hellobacsi(CrawlSpider):
    '''Crawl tin tức từ https://hellobacsi.com/ website
        limit: Giới hạn số trang để crawl, có thể bỏ trống.
    '''
    name = "hellobacsi"
    folder_path = "hellobacsi"
    page_limit = None
    start_urls = [
        URL
    ]
    ...
    def __init__(self, limit=-1, *args, **kwargs):
        super(Hellobacsi, self).__init__(*args, **kwargs)
    ...
    def extract_title(self, response):
        title = response.css("h1.article-header::text").extract_first() 
        if title is not None and len(title) > 0:
            return title
        return None

    def extract_description(self, response):
        description = response.css("div.body-content>p>b::text").extract_first()
