...
class Vinmec(CrawlSpider):
    '''Crawl tin tức từ https://vinmec.com website
    ...
    def __init__(self, limit=-1, *args, **kwargs):
        super(Vinmec, self).__init__(*args, **kwargs)
    ...
    def extract_description(self, response):
        news = response.css("div.block-content")
        description = news.css("b").extract_first()
        if description is not None and len(description) > 0:
            return description[0]
        return None

    def extract_content(self, response):
        content = response.css("div.block-content").getall()
